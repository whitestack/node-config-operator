...
const (
	grubCfgPath     = "/host/boot/grub/grub.cfg"
	grubDConfigPath = "/host/etc/default/grub.d/70_kernel_grub_nco.cfg"
	beginMarker     = "# BEGIN MARKER NCO GRUB CONFIG"
	endMarker       = "# END MARKER NCO GRUB CONFIG"
)
...
func (gkc GrubKernelConfig) applyModule() error {
	...
	if desiredBlock != "" {
		desiredBlock = fmt.Sprintf("%s\n%s\n%s\n", beginMarker, desiredBlock, endMarker)
	}
...
func (gkc GrubKernelConfig) findKernelEntry() (string, error) {
	output, err := execChroot("grep", "-m1", gkc.KernelVersion, grubCfgPath)
	if err != nil {
	...
	for _, line := range lines {
		if strings.Contains(line, gkc.KernelVersion) {
			start := strings.Index(line, "'")
			end := strings.LastIndex(line, "'")
			if start != -1 && end != -1 && start < end {
				entry := line[start+1 : end]
				if strings.Contains(entry, "(recovery mode)") {
					continue
				}
